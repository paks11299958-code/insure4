...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then(r => r.json()).then(u => setAuthUser(u))
  }, [])
    ...
    <main className="min-h-screen bg-[#2c2a29] text-[#c4b49a]">
      ...
      <div className="max-w-3xl mx-auto px-5 py-10 pb-20">
        ...
        <header className="text-center mb-10">
          <div className="inline-flex items-center gap-2.5 bg-[#d4b483]/[0.08] border border-[#d4b483]/25 rounded-full px-5 py-2 text-[17px] text-[#d4b483] tracking-widest mb-5">
            <Bell className="animate-bounce text-red-500 fill-red-500" size={20} />
            AI인공지능 중복 보험 분석 및 맞춤 컨설팅
          </div>
              ...
              <label className="flex items-center gap-1.5 cursor-pointer select-none">
                <input
                  ...
                  onChange={e => {
                    ...
                    setUserInfo(p => ({...p, title: e.target.checked ? '내보험 컨설팅' : ''}))
                  }}
                  className="w-3 h-3 accent-[#d4b483] cursor-pointer"
                />
                <span className="text-xs text-[#d4b483]">문구자동</span>
              </label>
            </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <label className="flex items-center gap-1.5 cursor-pointer select-none">
                  <input
                    ...
                    checked={autoPurpose}
                    onChange={e => {
                      setAutoPurpose(e.target.checked)
                      setUserInfo(p => ({...p, purpose: e.target.checked ? '중복 보장제거 및 컨설팅' : ''}))
                    }}
                    className="w-3 h-3 accent-[#d4b483] cursor-pointer"
                  />
                  <span className="text-xs text-[#d4b483]">문구자동</span>
        ...
        >
          ...
          <p className="text-xs text-[#7a7060]">PDF · JPG · PNG · TXT · DOCX 지원 &nbsp;·&nbsp; 최대 5개 파일</p>
          <p className="text-xs text-[#d4b483]/60 mt-1">※ 첨부파일은 최대 5개까지 업로드 가능합니다</p>
        </div>
